...
public class DataSet {
    ...
    public static String[] answerSet = {
            ...
            "\\item    C. App Engine flexible environments allow you to run containers on the App Engine PaaS. Kubernetes Engine is an orchestration platform for running containers. Both provide con- tainer management services. The App Engine standard environment runs applications in language-specific sandboxes and is not a general container management system. Cloud Functions is a serverless service for running code in response to events. It does not provide container services.",
            ...
            "\\item   . B. Bigtable is designed to accept billions of rows of data. Collecting data from 100,000 sensors every 5 seconds will generate 6,000,000 data points every minute, or 8,640,000,000 data points per day. Spanner is a relational database and supports transac- tions, but they are not needed. Cloud SQL MySQL and Cloud SQL PostgreSQL would be difficult to scale to this level of read and write performance.",
            ...
            "\\item   . B. Option B is correct. Spanner supports ANSI 2011 standard SQL and global transac- tions. Cloud SQL supports standard SQL but does not have global transaction. Datastore and Bigtable are NoSQL databases.",
            ...
            "\\item    D. Primitive roles only include the Owner, Editor, and View permissions. Predefined roles are designed for GCP products and services, like App Engine and BigQuery. For a custom application, you can create sets of privileges that give the user with that role as much per- mission as needed but not more.",
            ...
            "\\item   . B. In option B, the correct answer, the billing account is used to specify payment infor- mation and should be used to set up automatic payments. Service accounts are used to grant privileges to a VM and are not related to billing and payments. Resource accounts and credit accounts do not exist.",
            ...
            "\\item   . B. By default all users in an organization can create projects, which makes option B cor- rect. The role resourcemanager.projects.create is the role that allows users to create projects. The billing account is not associated with creating projects.",
            ...
            "\\item   . B. Compute Engine Admin Role is the role that gives users complete control over instances. Options A and C are fictitious roles. Compute Engine Security Admin gives users the privi- leges to create, modify, and delete SSL certificates and firewall rules.",
            ...
            "\\item   . C. The C programming language is not supported in the App Engine standard environ- ment. If you need to run a C application, it can be compiled and run in a container running in the App Engine flexible environment.",
            ...
            "\\item    A. You will need to set up billing if it is not already enabled when you start using the con- sole, so option A is the right answer. You may create a project, but you will be able to do this only if billing is enabled. You do not need to create a storage bucket to work with the console. Specifying a default zone is not a one-time task; you may change zones throughout the life of your project.",
            ...
            "\\item    B. The Reset command can be used to restart a VM; thus, option B is correct. The prop- erties of the VM will not change, but data in memory will be lost. There is no Reboot, Restart, Shutdown, or Startup option in the console.",
            "\\item    C. Labels, members of a managed instance group, and status are all available for filtering, so option C is the correct answer. You can also filter by internal IP, external IP, zone, net- work, deletion protection, and member of a managed or unmanaged instance group.",
            ...
            "\\item   . B. Deprecated marks the image as no longer supported and allows you to specify a replace- ment image to use going forward, making option B the correct answer. Deprecated images are available for use but may not be patched for security flaws or have other updates. The other options are fictitious features of images.",
            ...
            "\\item   . B. An instance group template is used to specify how the instance group should be created, which makes option B the correct answer. Option A is incorrect because instances are cre- ated automatically when an instance group is created. Boot disk images and snapshots do not have to be created before creating an instance group.",
            ...
            "\\item    C. Kubernetes creates instance groups as part of the process of creating a cluster, which makes option C the correct answer. Stackdriver, not instance groups, is used to monitor the health of nodes and to create alerts and notifications. Kubernetes creates pods and deploy- ments; they are not provided by instance groups.",
            ...
            "\\item   . B. Workspaces are logical structures for storing information about resources in a project that are being monitored, so option B is correct. Stackdriver works with logs, but a log is not required before starting to use Stackdriver. Pods and ReplicaSets are part of Kuber- netes, not Stackdriver.",
            "\\item   . C. The Stackdriver Instance Detail page includes time-series charts on CPU usage, network traffic, and disk I/O.",
            "\\item   . B. When creating an alert policy, you can specify conditions, notifications, and documenta- tion, making option B the correct answer. Options A and D are incorrect because there is no Time to Live attribute on policies. Option C is wrong because it does not include notifi- cations and documentation.",
            ...
            "\\item    B. When on the Cloud Console pages, you can click the cluster name to see a Details page, so option B is the correct answer. Typing the name of cluster in the search bar does not always return cluster details; it can return instance group details. There is no such com- mand as gcloud cluster details.",
            ...
            "\\item    C. Deployments are listed under Workloads, making option C the correct answer. The Cluster option shows details about clusters but does not have details on deployments. Stor- age shows information about persistent volumes and storage classes. Deployments is not an option.",
            ...
            "\\item   . A. The Deployment Details page includes services, so option A is the correct answer. Con- tainers are used to implement services; service details are not available there. The Clusters Detail page does not contain information on services running in the cluster.",
            ...
            "\\item   . B. Cloud engineers working with Kubernetes will need to be familiar with working with clusters, nodes, pods, and container images. They will also need to be familiar with deploy- ment. Option B is the correct answer because the other options are all missing an important component of Kubernetes that cloud engineers will have to manage.",
            ...
            "\\item    D. HTTP requests using GET, POST, DELETE, PUT, and OPTIONS can invoke an HTTP trigger in Cloud Functions, so option C is the right answer.",
            ...
            "\\item   . C. The three parameters are runtime, trigger-resource, and trigger-event, as listed in option C. All must be set, so options A and B are incorrect. file-type is not a param- eter to creating a cloud function on Cloud Storage, so option D is incorrect.",
            ...
            "\\item   . A. The HTTP trigger allows for the use of POST, GET, and PUT calls, so option A is the correct answer. Webhook and Cloud HTTP are not valid trigger types. Option D is incor- rect because option A is the correct answer.",
            ...
            "\\item   . B. Datastore is a document database, which makes option B correct. Cloud SQL and Span- ner are relational databases. Bigtable is a wide-column database. Google does not offer a managed graph database.",
            ...
            "\\item   . B. Firestore is a document database that has mobile supporting features, like data synchro- nization, so option B is the right answer. BigQuery is for analytics, not mobile or transac- tional applications. Spanner is a global relational database but does not have mobile-specific features. Bigtable could be used with mobile devices, but it does not have mobile-specific features like synchronization.",
            "\\item   . D. In addition to read and write patterns, cost, and consistency, you should consider trans- action support and latency, which makes option D correct.",
            ...
            "\\item   . A. Access charges are used with nearline and coldline storage, which makes option A cor- rect. There is no transfer charge involved. Options C and D do not refer to actual storage classes.",
            ...
            "\\item   . B. The command in option B correctly renames an object from an old name to a new name. Option A is incorrect because it uses a cp command instead of mv. Option C does not include bucket names, so it is incorrect. Option D uses gcloud, but gsutil is the com- mand-line tool for working with Cloud Storage.",
            ...
            "\\item    C. Option C is the correct command, gcloud sql export sql, indicating that the service is Cloud SQL, the operation is export, and the export file format is SQL. The filename and tar- get bucket are correctly formed. Option A is incorrect because it references gcloud storage, not gcloud sql. Option B is incorrect because it is missing an export file format parameter. Option D is incorrect because the bucket name and filename are in the wrong order.",
            "\\item    A. Option A uses the correct command, which is gcloud datastore export followed by a namespace and a bucket name. Option B is incorrect because the bucket name is miss- ing gs://. Options C and D are incorrect because they use the command dump instead of export. The bucket name in option D is missing gs://.",
            ...
            "\\item   . A. The correct command is bq load in option A. The autodetect and source_format parameters and path to source are correctly specified in all options. Option B is incorrect because it uses the term import instead of load. Options C and D are incorrect because they use gcloud instead of bq.",
            ...
            "\\item   . C. The correct answer, option C, uses gcloud pubsub subscriptions create followed by the topic and the name of the subscription. Option A is incorrect because it is missing the term subscriptions. Option B is incorrect because it is missing the name of the sub- scription. Option D is incorrect because it uses gsutil instead of gcloud.",
            ...
            "\\item    A. The correct answer is the Networking tab of the Management, Security, Disks, Net- working, Sole Tenancy section of the form, which makes option A correct. The Manage- ment tab is not about subnet configurations. Option D is incorrect because it does not lead to Sole Tenancy options.",
            ...
            "\\item   . B. The product you are working with is compute and the resource you are creating is a fire- wall rule, so option B is correct. Options A and C references network instead of compute. Option D references rules instead of firewall-rules.",
            ...
            "\\item   . C. The VPC create option is available in the Hybrid Connectivity section, so option C is cor- rect. Compute Engine, App Engine, and IAM & Admin do not have features related to VPNs.",
            ...
            "\\item   . A. Option A is correct because global dynamic routing is used to learn all routes on a net- work. Option B is incorrect; regional routing would learn only routes in a region. Options C and D are incorrect because they are not used to configure routing options.",
            ...
            "\\item    C.  The three global load balancers are HTTP(S), SSL Proxy, and TCP Proxy, so option C   is correct. Options A and B are missing at least one global load balancer. Option D is incor- rect because Internal TCP/UD is a regional load balancer.",
            ...
            "\\item    B. TCP Proxy load balancers require you to configure both the frontend and backendthe , so option B is correct. Options A and D are incorrect because they are missing one compo- nent. Option C is incorrect; forwarding rules are the one component specified with network load balancing. There is no component known as a traffic rule.",
            ...
            "\\item   . C. Premium is the network service level that routes all traffic over the Google network, so option C is correct. Option A is incorrect; the Standard tier may use the public Inter- net when routing traffic. Options B and D are incorrect; there are no service tiers called Google-only or non-Internet.",
            ...
            "\\item   . B. Network Services is the section of Cloud Console that has the Cloud DNS console, so option B is correct. Option A is incorrect; Compute Engine does not have DNS manage- ment forms. Neither does option C, Kubernetes Engine. Option D is related to networking, but the services in Hybrid Connectivity are for services such as VPNs.",
            ...
            "\\item   . B. Configuration files define resources and start with the term resources, so option B is cor- rect. Options A, B, and C are all incorrect. Those terms do not start the configuration file.",
            ...
            "\\item    C. The correct answer is gcloud projects get-iam-policy ace-exam-project, so option C is correct. Option A is incorrect because the resource should be projects and not iam. Option B is incorrect; list does not provide detailed descriptions. Option D is incor- rect because iam and list are incorrectly referenced.",
            ...
            "\\item   . B. Scopes are permissions granted to VM instances, so option B is correct. Scopes in com- bination with IAM roles assigned to service accounts assigned to the VM instance deter- mine what operations the VM instance can perform. Options A and C are incorrect; scopes do not apply to storage resources. Option D is incorrect; scopes do not apply to subnets.",
            ...
            "\\item   . B. The correct answer, option B, is halted. There is no such standard log level status. Sta- tuses include Critical, Error, Warning, Info, and Debug.",
            ...
            "\\item   . B. The Google Cloud Status Dashboard at https://status.cloud.google.com/ has information on the status of GCP services, so option B is correct. Options A and B might lead to information, but they would take longer. Option C is not a link to a source of infor- mation on BigQuery.",
            "\\item   . B. Both Compute Engine and Kubernetes Engine will require details about the VMs’ con- figurations, so option B is correct. The other options are incorrect because BigQuery and Cloud Pub/Sub are serverless services.",
            "\\item   . C. Query pricing in BigQuery is based on the amount of data scanned, so option C is cor- rect. Option A is incorrect; the amount of data storage is specified in the Storage Pricing section. Option B is incorrect; query pricing is not based on the volume of data returned. Option D is incorrect because this is not related to Cloud Storage. Option D is incorrect because option C is correct.",
            ...
            "\\item   . B. OpenCensus is a library for developing custom metrics that can be used with Stackdriver Logging, so option B is correct. Option A is incorrect; Prometheus is an open source moni- toring tool, but it is not used to define custom metrics in Stackdriver Monitoring. Option C is incorrect; Grafana is a visualization tools for Prometheus. Option D is incorrect; Nagios is an open source monitoring and alerting service, but it is not used for defining custom metrics in Stackdriver Logging.",
